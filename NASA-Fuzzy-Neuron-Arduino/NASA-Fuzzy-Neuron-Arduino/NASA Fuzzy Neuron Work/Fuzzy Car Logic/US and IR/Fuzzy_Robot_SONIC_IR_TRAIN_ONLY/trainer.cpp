...
const double alpha = .000005; // Controls how fast the algorthim learns...largest value we can have without overshooting the cost min
...
void getTrainingSet() {
  ...
  int data[5];	// use location of spaces to seperate location of individual datapoints, then convert the strings to integers
  for (int i = 0; i < 5; i++) {
    if (i == 0) // edge case
      data[i] = atoi(inString.substr(0, indecies[1]).c_str()); 
    else if (i == 4) // edge case
  ...
  inputs[1] = data[1];
  inputs[2] = data[2];
  expectedOutputs[0] = data[3];
  expectedOutputs[1] = data[4];
}
...
void fuzzy() {
  ...
  if(testy) {
  	cout << inputs[0] << " " << inputs[1] << " " << inputs[2] << " " << outputGuessM1 << " " << outputGuessM2 << endl;
  	return;
  }	
